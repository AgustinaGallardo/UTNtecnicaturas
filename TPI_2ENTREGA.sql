...
select  est.legajo_est 'Legajo',ape_est + ', ' + nom_est 'Nombre', 
		...
		est.calle + 'Nro ' + str(est.nro_calle) +' Barrio '+ b.barrio 'Direccion',
...
CAST(AVG(calificacion)AS DECIMAL (4,2)) 'Promedio',el.estado_lab 'EstadoLaboral'
from ESTUDIANTES e join ESTUDIANTES_MATERIAS em on e.legajo_est = em.legajo_est
join NACIONALIDADES n on e.id_nacionalidad = n.id_nacionalidad
join MATERIAS m on em.id_materia = m.id_materia
join TECNICATURAS_MATERIAS tm on m.id_materia = tm.id_materia
...
begin
 select e.legajo_est 'Legajo', nom_est+SPACE(1)+ape_est 'Estudiante', t.tecnicatura 'Tecnicatura',
CAST(avg(calificacion) AS DECIMAL(4,2)) 'Promedio'
from ESTUDIANTES e join ESTUDIANTES_MATERIAS em on e.legajo_est = em.legajo_est
join MATERIAS m on em.id_materia = m.id_materia
join TECNICATURAS_MATERIAS tm on m.id_materia = tm.id_materia
join TECNICATURAS t on tm.id_tecnicatura = t.id_tecnicatura
...
GO
create procedure Sp_Alumno_materias_informacion
@legajo int

as
begin
select 
mat.materia 'Materia',cur.curso 'Curso', cua.cuatrimestre 'Cuatrimestre',
 docent.ape_doc+','+docent.nom_doc 'Docente',
exmat.calificacion 'Calificacion'

from ESTUDIANTES_MATERIAS estmat
inner join MATERIAS mat on mat.id_materia=estmat.id_materia
inner join CURSOS cur on cur.id_curso=estmat.id_curso
inner join CUATRIMESTRES cua on cua.id_cuatrimestre=estmat.id_cuatrimestre
inner join DOCENTES_MATERIAS doc on doc.id_materia=estmat.id_materia
inner join DOCENTES docent on docent.legajo_doc=doc.legajo_doc
inner join EXAMEN_MATERIAS exmat on exmat.id_materia=estmat.id_materia
inner join ESTUDIANTES est on est.legajo_est=estmat.legajo_est
where @legajo=estmat.legajo_est and  @legajo=exmat.legajo_est
end

exec Sp_Alumno_materias_informacion 1
